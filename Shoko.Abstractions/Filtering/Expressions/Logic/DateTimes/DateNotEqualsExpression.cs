...
public class DateNotEqualsExpression : FilterExpression<bool>, IWithDateSelectorParameter, IWithSecondDateSelectorParameter, IWithDateParameter
{
    ...
    public override bool Evaluate(IFilterableInfo filterable, IFilterableUserInfo? userInfo, DateTime? time)
    {
        ...
        return (date!.Value > operand!.Value ? date - operand : operand - date).Value.TotalDays >= 1;
    }
    ...
    public override bool IsType(FilterExpression? expression)
        => expression is DateNotEqualsExpression exp && (Left?.IsType(exp.Left) ?? true) && (Right?.IsType(exp.Right) ?? true);
}
