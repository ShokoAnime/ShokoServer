...
*/
		...
		private static void ThrowIOError(int errorCode, string path)
		{
			switch (errorCode)
			{
				...
				case 15: // Drive not found
				{
					...
					throw new DriveNotFoundException(string.Format("Could not find the drive '{0}'. The drive might not be ready or might not be mapped.", path));
				}
				case 32: // Sharing violation
				{
					...
					throw new IOException(string.Format("The process cannot access the file '{0}' because it is being used by another process.", path), MakeHRFromErrorCode(errorCode));
				...
				case 80: // File already exists
				{
					...
					{
						throw new IOException(string.Format("The file '{0}' already exists.", path), MakeHRFromErrorCode(errorCode));
				...
				case 183: // File or directory already exists
				{
					if (!string.IsNullOrEmpty(path))
					{
						throw new IOException(string.Format("Cannot create '{0}' because a file or directory with the same name already exists.", path), MakeHRFromErrorCode(errorCode));
		...
		public static IList<Win32StreamInfo> ListStreams(string filePath)
		{
			...
			if (-1 != filePath.IndexOfAny(Path.GetInvalidPathChars())) throw new ArgumentException("The specified stream name contains invalid characters.", nameof(filePath));
