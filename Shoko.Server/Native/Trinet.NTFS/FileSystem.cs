...
namespace Trinet.Core.IO.Ntfs
{
	...
	public static class FileSystem
	{
		...
		public static AlternateDataStreamInfo GetAlternateDataStream(this FileSystemInfo file, string streamName, FileMode mode)
		{
			...
			{
				throw new NotSupportedException(string.Format("The specified mode '{0}' is not supported.", mode));
			...
			{
				throw new IOException(string.Format("The specified alternate data stream '{0}' does not exist on file '{1}'.", streamName, file.Name));

            }
			if (exists && FileMode.CreateNew == mode)
			{
				throw new IOException(string.Format("The specified alternate data stream '{0}' already exists on file '{1}'.", streamName, file.Name));
