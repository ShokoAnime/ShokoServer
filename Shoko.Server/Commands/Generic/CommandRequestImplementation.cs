    ...
    public abstract class CommandRequestImplementation : ICommandRequest
    {
        ...
        protected string TryGetProperty(XmlDocument doc, string keyName, string propertyName)
        {
            try
            {
                ...
                var parent = doc?.Cast<XmlNode>().FirstOrDefault(a => a.Name.ToLowerInvariant().Replace("_", "").Equals(keyTemp));
                if (parent == null) return string.Empty;

                var propName = propertyName.ToLowerInvariant().Replace("_", "");
                var prop = parent.Cast<XmlNode>().FirstOrDefault(a => a.Name.ToLowerInvariant().Replace("_", "").Equals(propName))?.InnerText.Trim();
