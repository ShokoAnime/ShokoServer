...
public static class FilterExtensions
{
    ...
    public static IFilterable ToFilterable(this SVR_AnimeGroup group)
    {
        ...
        var filterable = new Filterable
        {
            ...
            LowestAniDBRating = group.Anime.DefaultIfEmpty().Min(anime => decimal.Round(Convert.ToDecimal(anime?.Rating ?? 00) / 100, 1, MidpointRounding.AwayFromZero)),
            HighestAniDBRating = group.Anime.DefaultIfEmpty().Max(anime => decimal.Round(Convert.ToDecimal(anime?.Rating ?? 00) / 100, 1, MidpointRounding.AwayFromZero)),
            ...
            VideoSources = group.Contract?.Stat_AllVideoQuality_Episodes ?? new HashSet<string>(),
    ...
    public static IFilterable ToUserDependentFilterable(this SVR_AnimeGroup group, int userID)
    {
        var series = group.GetAllSeries();
        var hasTrakt = series.All(a => RepoFactory.CrossRef_AniDB_TraktV2.GetByAnimeID(a.AniDB_ID).Any());
