        ...
        public TvDB_Episode TvDBEpisode
        {
            get
            {
                using (var session = DatabaseFactory.SessionFactory.OpenSession())
                {
                    ...
                    TvDB_Episode tvep =
                        RepoFactory.TvDB_Episode.GetBySeriesIDSeasonNumberAndEpisode(session, xref_tvdb2.TvDBID, season,
                            epnumber);
                    if (tvep != null) return tvep;

                    int lastSeason = RepoFactory.TvDB_Episode.getLastSeasonForSeries(session, xref_tvdb2.TvDBID);
                    int previousSeasonsCount = 0;
                    // we checked once, so increment the season
                    season++;
                    previousSeasonsCount += RepoFactory.TvDB_Episode.GetNumberOfEpisodesForSeason(session, xref_tvdb2.TvDBID,
                        season);
                    do
                    {
                        ...
                        tvep = RepoFactory.TvDB_Episode.GetBySeriesIDSeasonNumberAndEpisode(session, xref_tvdb2.TvDBID,
                            season, epnumber - previousSeasonsCount);
