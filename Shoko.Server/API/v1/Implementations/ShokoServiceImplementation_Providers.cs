...
public partial class ShokoServiceImplementation
{
    ...
    public CL_AniDB_AnimeCrossRefs GetCrossRefDetails(int animeID)
    {
        ...
        try
        {
            ...
            if (xrefMovie?.TmdbMovie is { } tmdbMovie)
            {
                ...
                foreach (var fanart in ((IWithImages)tmdbMovie).GetImages(new() { ImageType = ImageEntityType.Backdrop }))
                    result.MovieDBFanarts.Add(fanart.ToClientFanart());
                foreach (var poster in ((IWithImages)tmdbMovie).GetImages(new() { ImageType = ImageEntityType.Primary }))
    ...
    public List<CL_Trakt_Episode> GetAllTraktEpisodes(int? traktShowID)
    {
        return [];
    }

    [HttpGet("Trakt/Episode/FromTraktId/{traktID}")]
    public List<CL_Trakt_Episode> GetAllTraktEpisodesByTraktID(string traktID)
    {
        return [];
    }
    ...
    public List<CL_CrossRef_AniDB_TraktV2> GetTraktCrossRefV2(int animeID)
    {
        return [];
    }

    [HttpGet("Trakt/CrossRef/Episode/{animeID}")]
    public List<object> GetTraktCrossRefEpisode(int animeID)
    {
        return [];
    }
    ...
    public List<CL_CrossRef_AniDB_TraktV2> GetAllTraktCrossRefs()
    {
        return [];
    }

    [HttpGet("Trakt/Comment/{animeID}")]
    public List<object> GetTraktCommentsForAnime(int animeID)
    {
        return [];
    }
    ...
    public CL_CrossRef_AniDB_Other GetOtherAnimeCrossRef(int animeID, int crossRefType)
    {
        try
        {
