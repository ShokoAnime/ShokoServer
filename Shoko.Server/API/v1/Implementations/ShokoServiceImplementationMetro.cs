    ...
    public class ShokoServiceImplementationMetro : IShokoServerMetro, IHttpContextAccessor
    {
        ...
        public List<Metro_Anime_Summary> GetAnimeWithNewEpisodes(int maxRecords, int jmmuserID)
        {
            ...
            try
            {
                { 
                    ...
                    IDictionary<int, DateTime> results = Repo.Instance.VideoLocal.GetEpisodesRecentlyAdded();
        ...
        public List<Metro_Anime_Summary> GetAnimeContinueWatching(int maxRecords, int jmmuserID)
        {
            ...
            try
            {
                ...
                {
                    ShokoServiceImplementation imp = new ShokoServiceImplementation();
                    foreach (SVR_AnimeSeries ser in Repo.Instance.AnimeSeries.GetByGroupID(grp.AnimeGroupID))
                    {
                        if (!user.AllowedSeries(ser)) continue;
            ...
            int maxRecords)
        {
            ...
            try
            {
                ...
                List<SVR_AniDB_Anime> animes =
                    Repo.Instance.AniDB_Anime.GetForDate(startDate.Value, endDate.Value);
                foreach (SVR_AniDB_Anime anidb_anime in animes)
                {
                    if (!user.AllowedAnime(anidb_anime)) continue;

                    SVR_AnimeSeries ser = Repo.Instance.AnimeSeries.GetByAnimeID(anidb_anime.AnimeID);
        ...
        public List<Metro_Anime_Summary> SearchAnime(int jmmuserID, string queryText, int maxRecords)
        {
            ...
            try
            {
                ...
                List<SVR_AniDB_Anime> animes = Repo.Instance.AniDB_Anime.SearchByName(queryText);
                foreach (SVR_AniDB_Anime anidb_anime in animes)
                {
                    if (!user.AllowedAnime(anidb_anime)) continue;
            ...
            try
            {
                ...
                if (ser != null)
                {
                    ...
                    foreach (
                        ...
                        Repo.Instance.AnimeEpisode_User.GetByUserIDAndSeriesID(jmmuserID, ser.AnimeSeriesID))
                        dictEpUsers[userRecord.AnimeEpisodeID] = userRecord;

                    foreach (SVR_AnimeEpisode animeep in Repo.Instance.AnimeEpisode.GetBySeriesID(ser.AnimeSeriesID))
                    {
                    ...
                    List<AniDB_Episode> aniEpList = Repo.Instance.AniDB_Episode.GetByAnimeID(ser.AniDB_ID);
        ...
        public List<Metro_AniDB_Character> GetCharactersForAnime(int animeID, int maxRecords)
        {
            ...
            try
            {
                ...
                {
                    cnt++;
                    AniDB_Character chr = Repo.Instance.AniDB_Character.GetByID(animeChar.CharID);
                    if (chr != null)
                    {
        ...
        public List<Metro_Anime_Summary> GetSimilarAnimeForAnime(int animeID, int maxRecords, int jmmuserID)
        {
            ...
            try
            {
                ...
                SVR_JMMUser juser = Repo.Instance.JMMUser.GetByID(jmmuserID);
                ...
                foreach (AniDB_Anime_Similar link in anime.GetSimilarAnime())
                {
                    SVR_AniDB_Anime animeLink =
                        Repo.Instance.AniDB_Anime.GetByID(link.SimilarAnimeID);
            ...
            bool watchedStatus, int userID)
        {
            ...
            try
            {
                ...
                // refresh from db
                ep = Repo.Instance.AnimeEpisode.GetByID(animeEpisodeID);
