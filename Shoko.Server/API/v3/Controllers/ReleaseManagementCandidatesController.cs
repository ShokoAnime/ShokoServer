...
/// <summary>
/// Provides release-candidate ranking, preview, and deletion for series with
/// multiple competing releases. Candidates are ranked by the configured quality
/// signals; the highest-ranked candidate is the primary and lower-ranked
/// candidates that are fully covered by it are considered redundant.
/// </summary>
[ApiController]
[Route("/api/v{version:apiVersion}/ReleaseManagement/Candidates")]
[ApiV3]
[Authorize]
public class ReleaseManagementCandidatesController(
...
) : BaseController(settingsProvider)
{
    ...
    [HttpGet("Series")]
    public ActionResult<ListResult<SeriesWithCandidates>> GetSeriesWithMultipleCandidates(
    ...
    /// <summary>
    /// Get the ranked release candidates for a specific series.
    /// </summary>
    ...
    /// <summary>
    /// Compute a preview of which files would be deleted across all series
    /// with redundant candidates. Optionally exclude specific series or
    /// override which candidate is treated as the primary for a given series.
    /// </summary>
    /// <remarks>
    ...
    {
        var excludedIDs = body?.ExcludedSeriesIDs?.ToHashSet() ?? [];
        var overrides = body?.Overrides?.ToDictionary(o => o.SeriesID, o => o.PreferredCandidateKey)
                        ?? new Dictionary<int, string>();

        var result = animeSeries.GetAll()
            .Where(s => User.AllowedSeries(s) && !excludedIDs.Contains(s.AnimeSeriesID))
        ...
        Shoko.Server.Models.Shoko.AnimeSeries series)
    {
        ...
        var isAiring = autoManagement.IsSeriesAiring(series);
        ...
        Dictionary<int, string> overrides)
    {
        ...
        // Apply per-series override: move the preferred candidate to rank 1.
