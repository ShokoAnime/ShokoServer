    ...
    public static class ModelHelper
    {
        ...
        public static SeriesSizes GenerateSeriesSizes(List<SVR_AnimeEpisode> episodeList, int userID)
        {
            ...
            foreach (var episode in episodeList)
            {
                ...
                bool isWatched = (episode.GetUserRecord(userID)?.WatchedCount ?? 0) > 0;
                ...
                {
                    sizes.Total.Unknown++;
                    if (isLocal) sizes.Local.Unknown++;
                    if (isWatched) sizes.Watched.Unknown++;
                    continue;
                }
                switch ((EpisodeType)anidbEpisode.EpisodeType)
                {
                    case EpisodeType.Episode:
                        ...
                        if (isLocal) sizes.Local.Episodes++;
                        if (isWatched) sizes.Watched.Episodes++;
                        break;
                    case EpisodeType.Credits:
                        sizes.Total.Credits++;
                        if (isLocal) sizes.Local.Credits++;
                        if (isWatched) sizes.Watched.Credits++;
                        break;
                    case EpisodeType.Special:
                        sizes.Total.Specials++;
                        if (isLocal) sizes.Local.Specials++;
                        if (isWatched) sizes.Watched.Specials++;
                        break;
                    case EpisodeType.Trailer:
                        sizes.Total.Trailers++;
                        if (isLocal) sizes.Local.Trailers++;
                        if (isWatched) sizes.Watched.Trailers++;
                        break;
                    case EpisodeType.Parody:
                        sizes.Total.Parodies++;
                        if (isLocal) sizes.Local.Parodies++;
                        if (isWatched) sizes.Watched.Parodies++;
                        break;
                    case EpisodeType.Other:
                        sizes.Total.Others++;
                        if (isLocal) sizes.Local.Others++;
                        if (isWatched) sizes.Watched.Others++;
                        break;
                }
            }
            return sizes;
