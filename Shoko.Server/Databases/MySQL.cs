...
public class MySQL : BaseDatabase<MySqlConnection>, IDatabase
{
    ...
    public int RequiredVersion { get; } = 114;
    ...
    private List<DatabaseCommand> patchCommands = new()
    {
        ...
        new(99, 2, "UPDATE AniDB_File SET IsChaptered = 0 WHERE IsChaptered = -1; UPDATE AniDB_File SET IsDeprecated = 0 WHERE IsDeprecated = -1;"),
        new(99, 3, "ALTER TABLE `AniDB_File` MODIFY `IsCensored` bit NULL; ALTER TABLE `AniDB_File` MODIFY `IsDeprecated` bit not null; ALTER TABLE `AniDB_File` MODIFY `IsChaptered` bit not null"),
        new(99, 4, "ALTER TABLE `AniDB_GroupStatus` MODIFY `Rating` decimal(6,2) NULL; UPDATE `AniDB_GroupStatus` SET `Rating` = `Rating` / 100 WHERE `Rating` > 10"),
        new(99, 5, "ALTER TABLE `AniDB_Character` DROP COLUMN CreatorListRaw;"),
        new(99, 6, "ALTER TABLE `AniDB_Anime_Character` DROP COLUMN EpisodeListRaw;"),
        new(99, 7, "ALTER TABLE `AniDB_Anime` DROP COLUMN AwardList;"),
        new(99, 8, "ALTER TABLE `AniDB_File` DROP COLUMN AnimeID;"),
