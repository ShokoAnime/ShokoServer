...
public class FilterSelectorConverter : TypeConverter, IUserType
{
    private static Dictionary<string, IFilterSelector> s_selectors = new();
    
    public override bool CanConvertFrom(ITypeDescriptorContext context, Type sourceType)
    {
        return typeof(IFilterSelector).IsAssignableFrom(sourceType);
    }
        ...
        object value)
    {
        ...
        {
            case string s:
                if (s_selectors.TryGetValue(s, out var selector)) return selector;

                var type = Type.GetType(s) ?? throw new ArgumentException($"Value was not found: {s}");
                selector = (IFilterSelector)Activator.CreateInstance(type);
                s_selectors.Add(s, selector);
                return selector;
            default:
                throw new ArgumentException("DestinationType must be string");
        }
    }
        ...
        object value, Type destinationType)
    {
        ...
        if (value is not IFilterSelector)
        {
            throw new ArgumentException(@"Value isn't of type IFilterSelector", nameof(value));
        }

        return destinationType.FullName switch
        {
            "System.String" => value.GetType().FullName,
