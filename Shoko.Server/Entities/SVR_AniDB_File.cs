    ...
    public class SVR_AniDB_File : AniDB_File
    {
        ...
        public List<int> EpisodeIDs
        {
            get
            {
                ...
                List<SVR_CrossRef_File_Episode> fileEps = RepoFactory.CrossRef_File_Episode.GetByHash(this.Hash);

                foreach (SVR_CrossRef_File_Episode crossref in fileEps)
                {
        ...
        [XmlIgnore]
        public List<SVR_AniDB_Episode> Episodes
        {
            get
            {
                List<SVR_AniDB_Episode> eps = new List<SVR_AniDB_Episode>();
        ...
        public string EpisodesRAW
        {
            get
            {
                if (!string.IsNullOrEmpty(episodesRAW))
                    return episodesRAW;
                string ret = "";
                foreach (SVR_CrossRef_File_Episode cross in EpisodeCrossRefs)
                {
                    if (ret.Length > 0)
                        ret += ", ";
        ...
        public void CreateCrossEpisodes(string localFileName)
        {
            if (episodesRAW != null) //Only create relations if the origin of the data if from Raw (AniDB)
            {
                ...
                foreach (SVR_CrossRef_File_Episode fileEp in fileEps)
                ...
                for (int x = 0; x < epi.Length; x++)
                {
                    ...
                    if (epis.Length > 0)
                    {
                        ...
                        if (epid != 0)
                        {
                            SVR_CrossRef_File_Episode cross = new SVR_CrossRef_File_Episode();
                            cross.Hash = Hash;
