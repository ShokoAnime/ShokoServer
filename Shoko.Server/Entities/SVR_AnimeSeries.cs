    ...
    public class SVR_AnimeSeries : AnimeSeries
    {
            ...
            return
                RepoFactory.AnimeEpisode
                    .GetBySeriesID(AnimeSeriesID).Count(a => a.EpisodeTypeEnum == enEpisodeType.Episode &&
                            RepoFactory.CrossRef_File_Episode.GetByEpisodeID(RepoFactory.AniDB_Episode.GetByEpisodeID(a.AniDB_EpisodeID)?.EpisodeID ?? 0)
        ...
        public int GetAnimeNumberOfEpisodeTypes()
        {
            return RepoFactory.AnimeEpisode
                ...
                .Where(a => RepoFactory.CrossRef_File_Episode.GetByEpisodeID(RepoFactory.AniDB_Episode.GetByEpisodeID(a.AniDB_EpisodeID)?.EpisodeID ?? 0)
                    .Select(b => RepoFactory.VideoLocal.GetByHash(b.Hash))
        ...
        public int GetAnimeEpisodesCountWithVideoLocal()
        {
            ...
            return RepoFactory.AnimeEpisode.GetBySeriesID(AnimeSeriesID).Count(a => RepoFactory.CrossRef_File_Episode.GetByEpisodeID(RepoFactory.AniDB_Episode.GetByEpisodeID(a.AniDB_EpisodeID)?.EpisodeID ?? 0).Select(b => RepoFactory.VideoLocal.GetByHash(b.Hash))
                    .Count(b => b != null) > 0);
            /*
            using (var session = JMMService.SessionFactory.OpenSession())
            {
        ...
        public List<Trakt_Show> GetTraktShow(ISession session)
        {
            ...
            List<SVR_CrossRef_AniDB_TraktV2> xrefs = GetCrossRefTraktV2(session);
            if (xrefs == null || xrefs.Count == 0) return sers;
        ...
        public SVR_CrossRef_AniDB_Other CrossRefMovieDB
        {
            get
            {
                return RepoFactory.CrossRef_AniDB_Other.GetByAnimeIDAndType(this.AniDB_ID, CrossRefType.MovieDB);
            }
        }
        ...
        public List<CrossRef_AniDB_MAL> CrossRefMAL
        {
            get
            {
                return RepoFactory.CrossRef_AniDB_MAL.GetByAnimeID(this.AniDB_ID);
            }
        }
        ...
        public void CreateAnimeEpisodes(ISession session)
        {
            ...
            foreach (SVR_AniDB_Episode ep in anime.GetAniDBEpisodes())
            {
        ...
        public SVR_AnimeGroup AnimeGroup
        {
            get
            {
                return RepoFactory.AnimeGroup.GetByID(this.AnimeGroupID);
            }
        }
            ...
            IReadOnlyCollection<SVR_AnimeSeries> seriesBatch, bool onlyStats = false)
        {
            ...
            var tvDbByAnime = new Lazy<ILookup<int, Tuple<SVR_CrossRef_AniDB_TvDBV2, TvDB_Series>>>(
                () => RepoFactory.TvDB_Series.GetByAnimeIDsV2(session, animeIds.Value), false);
            var movieByAnime = new Lazy<Dictionary<int, Tuple<SVR_CrossRef_AniDB_Other, MovieDB_Movie>>>(
            ...
            foreach (SVR_AnimeSeries series in seriesBatch)
            {
                ...
                if (!seriesOnlyStats)
                {
                    ...
                    // MovieDB contracts
                    Tuple<SVR_CrossRef_AniDB_Other, MovieDB_Movie> movieDbInfo;
                    ...
                    // MAL contracts
                    contract.CrossRefAniDBMAL = malXrefByAnime.Value[series.AniDB_ID].Cast<Shoko.Models.Server.CrossRef_AniDB_MAL>().ToList();
                }
        ...
        public HashSet<GroupFilterConditionType> UpdateContract(bool onlystats = false)
        {
            ...
            List<SVR_CrossRef_AniDB_TvDBV2> tvDBCrossRefs = this.GetCrossRefTvDBV2();
            SVR_CrossRef_AniDB_Other movieDBCrossRef = this.CrossRefMovieDB;
            MovieDB_Movie movie = null;
            if (movieDBCrossRef != null)
                movie = movieDBCrossRef.GetMovieDB_Movie();
            List<TvDB_Series> sers = new List<TvDB_Series>();
            foreach (SVR_CrossRef_AniDB_TvDBV2 xref in tvDBCrossRefs)
            {
            ...
            CL_AnimeSeries_User newcontract)
        {
            ...
            if (oldcontract == null ||
                !oldcontract.AniDBAnime.AniDBAnime.GetAllTags().SetEquals(newcontract.AniDBAnime.AniDBAnime.GetAllTags()))
        ...
        public void UpdateStats(bool watchedStats, bool missingEpsStats, bool updateAllGroupsAbove)
        {
            ...
            Dictionary<int, List<SVR_CrossRef_File_Episode>> dictCrossRefs =
                new Dictionary<int, List<SVR_CrossRef_File_Episode>>();
            foreach (SVR_CrossRef_File_Episode xref in crossRefs)
            {
                if (!dictCrossRefs.ContainsKey(xref.EpisodeID))
                    dictCrossRefs[xref.EpisodeID] = new List<SVR_CrossRef_File_Episode>();
            ...
            if (watchedStats)
            {
                foreach (JMMUser juser in allUsers)
                {
                    ...
                    List<SVR_AnimeEpisode_User> epUserRecords = RepoFactory.AnimeEpisode_User.GetByUserID(juser.JMMUserID);
            ...
            if (missingEpsStats)
            {
                ...
                List<SVR_AniDB_GroupStatus> grpStatuses = RepoFactory.AniDB_GroupStatus.GetByAnimeID(this.AniDB_ID);
