    ...
    public IReadOnlyList<AniDB_ReleaseGroup> GetUsedReleaseGroups()
        => Lock(() =>
        {
            ...
            return session.Query<AniDB_ReleaseGroup>()
                ...
                .Join(session.Query<SVR_CrossRef_File_Episode>(), a => a.File.Hash, a => a.Hash, (a, b) => new { a.Group, a.File, Xref = b })
                .Join(session.Query<SVR_VideoLocal>(), a => a.File.Hash, a => a.Hash, (a, b) => a.Group)
