...
public class ServerSettings
{
    ...
    [JsonIgnore] public static string DefaultInstance { get; set; } = Assembly.GetEntryAssembly().GetName().Name;
    [JsonIgnore] public static string DefaultImagePath => Path.Combine(ApplicationPath, "images");
    ...
    public Guid GA_Client
    {
        get
        {
            if (Guid.TryParse(GA_ClientId, out var val))
            {
                return val;
            }
    ...
    public static ServerSettings Instance { get; private set; } = new();
    ...
    public static void LoadSettings()
    {
        ...
        if (!File.Exists(path))
        {
            Instance = File.Exists(Path.Combine(ApplicationPath, "settings.json"))
                ? LoadLegacySettings()
                : new ServerSettings();
