...
namespace JMMServer.Providers.TraktTV
{
    public class TraktTVHelper
    {
        ...
        public static void UpdateAllInfo()
        {
            ...
            {
                var cmd = new CommandRequest_TraktUpdateInfoAndImages(xref.TraktID);
                cmd.Save();
            }
        }
        ...
        public static void SyncCollectionToTrakt_Series(AnimeSeries series)
        {
            try
            {
                ...
                var traktUsers = repUsers.GetTraktUsers();
            ...
            {
                logger.ErrorException("Error in TraktTVHelper.SyncCollectionToTrakt_Series: " + ex, ex);
            }
        }
        ...
        public static void SyncCollectionToTrakt()
        {
            try
            {
                ...
                foreach (var series in allSeries)
                {
                    ...
                    foreach (var ep in series.GetAnimeEpisodes())
                    {
                        if (ep.EpisodeTypeEnum == enEpisodeType.Episode || ep.EpisodeTypeEnum == enEpisodeType.Special)
                        {
                            var epsync = ReconSyncTraktEpisode(series, ep, traktSummary, traktUsers, collected, watched,
                                false);
                            if (epsync != null)
                            {
                ...
                foreach (var col in collected)
                {
                    ...
                    var xrefs = repCrossRef.GetByTraktID(col.show.ids.slug);

                    if (xrefs.Count > 0)
                    {
                        foreach (var xref in xrefs)
                        {
                            var locSeries = repSeries.GetByAnimeID(xref.AnimeID);
                            ...
                            foreach (var ep in locSeries.GetAnimeEpisodes())
                            {
                                ...
                                {
                                    var epsync = ReconSyncTraktEpisode(locSeries, ep, traktSummary, traktUsers,
                ...
                foreach (var wtch in watched)
                {
                    ...
                    // check if we have this series locally
                    var xrefs = repCrossRef.GetByTraktID(wtch.show.ids.slug);
            ...
            {
                logger.ErrorException("Error in TraktTVHelper.SyncCollectionToTrakt: " + ex, ex);
            }
        }
        ...
        public static bool CheckTraktValidity(string slug, bool removeDBEntries)
        {
            try
            {
                ...
                var show = repShows.GetByTraktSlug(slug);
            ...
            {
                logger.ErrorException("Error in TraktTVHelper.CleanupDatabase: " + ex, ex);
                return false;
            }
        }
        ...
        {
            var repShows = new Trakt_ShowRepository();
            var repXRefs = new CrossRef_AniDB_TraktV2Repository();
            var repFanart = new Trakt_ImageFanartRepository();
            var repSeasons = new Trakt_SeasonRepository();
            var repEpisodes = new Trakt_EpisodeRepository();
            var repPosters = new Trakt_ImagePosterRepository();

            // this means Trakt has no record of this slug.
            // 1. Delete any cross ref links
            foreach (var xref in repXRefs.GetByTraktID(show.TraktID))
                repXRefs.Delete(xref.CrossRef_AniDB_TraktV2ID);

            // 2. Delete default image links

            // 3. Delete episodes
            foreach (var epTemp in repEpisodes.GetByShowID(show.Trakt_ShowID))
                repEpisodes.Delete(epTemp.Trakt_EpisodeID);

            // 4. Delete fanart and posters
            foreach (var fanart in repFanart.GetByShowID(show.Trakt_ShowID))
                repFanart.Delete(fanart.Trakt_ImageFanartID);

            foreach (var poster in repPosters.GetByShowID(show.Trakt_ShowID))
                repPosters.Delete(poster.Trakt_ImagePosterID);

            // 5. Delete seasons
            foreach (var season in repSeasons.GetByShowID(show.Trakt_ShowID))
        ...
        public static void CleanupDatabase()
        {
            try
            {
                ...
                foreach (var show in repShows.GetAll())
                {
                    // let's check if we can get this show on Trakt
                    var traktCode = TraktStatusCodes.Success;
            ...
            List<TraktV2ShowCollectedResult> collected, List<TraktV2ShowWatchedResult> watched, bool sendNow)
        {
            try
            {
                ...
                var msg1 = string.Format("Sync Check Status:  AniDB: {0} - {1} - {2} - Collection: {3} - Watched: {4}",
                ...
                if (localCollection)
                {
                    ...
                    if (!onlineCollection)
                    {
                        var msg = string.Format("SYNC LOCAL: Adding to Trakt Collection:  Slug: {0} - S:{1} - EP:{2}",
                ...
                if (localWatched)
                {
                    ...
                    if (!onlineWatched)
                    {
                        var msg = string.Format("SYNC LOCAL: Adding to Trakt History:  Slug: {0} - S:{1} - EP:{2}",
                            traktShowID, season, epNumber);
                        logger.Trace(msg);
                ...
                else
                {
                    ...
                    if (onlineWatched)
                    {
                        var msg = string.Format("SYNC LOCAL: Removing from Trakt History:  Slug: {0} - S:{1} - EP:{2}",
                            traktShowID, season, epNumber);
                        logger.Trace(msg);
            ...
            {
                logger.ErrorException("Error in TraktTVHelper.SyncTraktEpisode: " + ex, ex);
                return null;
            }
        }
            ...
            ref List<TraktV2ShowWatchedResult> watched)
        {
            ...
            {
                logger.ErrorException("Error in TraktTVHelper.GetTraktCollectionInfo: " + ex, ex);
                return false;
            }
        }
            ...
            ref string webResponse)
        {
            ...
            try
            {
                ...
                var msg = "Trakt SEND Data" + Environment.NewLine +
        ...
        public static string GetFromTrakt(string uri, ref int traktCode)
        {
            ...
            var msg = "Trakt GET Data" + Environment.NewLine +
                      "uri: " + uri + Environment.NewLine;
            logger.Trace(msg);
        ...
        public static bool RefreshAuthToken()
        {
            try
            {
                ...
                var token = new TraktV2RefreshToken();
            ...
            catch (Exception ex)
            {
                ...
                logger.ErrorException("Error in TraktTVHelper.RefreshAuthToken: " + ex, ex);
                return false;
            }
        }
        ...
        public static string EnterTraktPIN(string pin)
        {
            ...
            catch (Exception ex)
            {
                ...
                logger.ErrorException("Error in TraktTVHelper.TestUserLogin: " + ex, ex);
                return ex.Message;
            }
        }
            ...
            string traktID, int seasonNumber, int traktEpNumber, bool excludeFromWebCache)
        {
            ...
            if (xrefTemps != null && xrefTemps.Count > 0)
            {
                foreach (var xrefTemp in xrefTemps)
                {
                    ...
                    RemoveLinkAniDBTrakt(xrefTemp.AnimeID, (enEpisodeType)xrefTemp.AniDBStartEpisodeType,
            ...
            var xref = repCrossRef.GetByTraktID(session, traktID, seasonNumber, traktEpNumber, animeID, (int)aniEpType,
                aniEpNumber);
            if (xref == null)
            ...
            {
                var req = new CommandRequest_WebCacheSendXRefAniDBTrakt(xref.CrossRef_AniDB_TraktV2ID);
                req.Save();
            }
        ...
        {
            var repCrossRef = new CrossRef_AniDB_TraktV2Repository();
            var xref = repCrossRef.GetByTraktID(traktID, seasonNumber, traktEpNumber, animeID, (int)aniEpType,
                aniEpNumber);
            if (xref == null) return;
            ...
            {
                var req = new CommandRequest_WebCacheDeleteXRefAniDBTrakt(animeID, (int)aniEpType, aniEpNumber,
            ...
            ref Dictionary<int, Trakt_Episode> dictTraktSpecials, ref Dictionary<int, int> dictTraktSeasons)
        {
            ...
            try
            {
                ...
                var eps = repEps.GetByShowID(show.Trakt_ShowID);
        ...
        public static void ScanForMatches()
        {
            ...
            foreach (var ser in allSeries)
            {
                ...
                var cmd = new CommandRequest_TraktSearchAnime(ser.AniDB_ID, false);
                cmd.Save();
            }
        }
        ...
        private static int? GetTraktEpisodeIdV2(AnimeEpisode ep, ref string traktID, ref int season, ref int epNumber)
        {
            ...
            var repAnime = new AniDB_AnimeRepository();
            var anime = repAnime.GetByAnimeID(aniep.AnimeID);
                ...
                if (ep.EpisodeTypeEnum == enEpisodeType.Episode)
                {
                        ...
                        traktSummary.CrossRefTraktV2.Count > 0)
                    {
                        ...
                        sortCriteria.Add(new SortPropOrFieldAndDirection("AniDBStartEpisodeNumber", true,
                            SortType.eInteger));
                        var traktCrossRef = Sorting.MultiSort(traktSummary.CrossRefTraktV2, sortCriteria);
        ...
        public static void UpdateAllInfoAndImages(string traktID, bool forceRefresh)
        {
            ...
            {
                logger.ErrorException("Error in TraktTVHelper.UpdateAllInfoAndImages: " + ex, ex);
            }
        }
        ...
        public static void DownloadAllImages(string traktID)
        {
            try
            {
                ...
                var show = repShow.GetByTraktSlug(traktID);
                if (show == null) return;
                ...
                if (ServerSettings.Trakt_DownloadFanart)
                {
                    ...
                    var repFanart = new Trakt_ImageFanartRepository();
                    var fanart = repFanart.GetByShowIDAndSeason(show.Trakt_ShowID, 1);
                    if (fanart != null)
                    {
                        if (!string.IsNullOrEmpty(fanart.FullImagePath))
                        {
                            ...
                            {
                                var cmd = new CommandRequest_DownloadImage(fanart.Trakt_ImageFanartID,
                ...
                foreach (var season in show.Seasons)
                {
                    ...
                    {
                        var poster = repPosters.GetByShowIDAndSeason(season.Trakt_ShowID, season.Season);
                        if (poster != null)
                        {
                            if (!string.IsNullOrEmpty(poster.FullImagePath))
                            {
                                ...
                                {
                                    var cmd = new CommandRequest_DownloadImage(poster.Trakt_ImagePosterID,
                    ...
                    if (ServerSettings.Trakt_DownloadEpisodes)
                    {
                        ...
                        foreach (var ep in season.Episodes)
                        {
                            if (!string.IsNullOrEmpty(ep.FullImagePath))
                            {
                                ...
                                {
                                    var cmd = new CommandRequest_DownloadImage(ep.Trakt_EpisodeID,
            ...
            ref string returnMessage)
        {
            ...
            {
                logger.ErrorException("Error in TraktTVHelper.PostCommentShow: " + ex, ex);
        ...
        public static void SyncEpisodeToTrakt(AnimeEpisode ep, TraktSyncType syncType, bool secondaryAction = true)
        {
            ...
            {
                logger.ErrorException("Error in TraktTVHelper.MarkEpisodeWatched: " + ex, ex);
            }
        }
            ...
            DateTime epDate, bool secondaryAction = true)
        {
            try
            {
                ...
                if (syncType == TraktSyncType.CollectionAdd || syncType == TraktSyncType.CollectionRemove)
                {
                    var sync = new TraktV2SyncCollectionEpisodesByNumber(slug, season, epNumber, epDate);
                    json = JSONHelper.Serialize(sync);
                }
                else
                {
                    var sync = new TraktV2SyncWatchedEpisodesByNumber(slug, season, epNumber, epDate);
                    json = JSONHelper.Serialize(sync);
                }
                ...
                var retData = string.Empty;
                var response = SendData(url, json, "POST", BuildRequestHeaders(), ref retData);
                /*if (response == TraktStatusCodes.Success || response == TraktStatusCodes.Success_Post || response == TraktStatusCodes.Success_Delete)
                {
                    // if this was marking an episode as watched, and is successful, let's also add this to the user's collection
                    // this is because you can watch something without adding it to your collection, but in JMM it is always part of your collection
            ...
            {
                logger.ErrorException("Error in TraktTVHelper.SyncEpisodeToTrakt: " + ex, ex);
            }
        }
            ...
            ScrobblePlayingStatus scrobbleStatus, float progress)
        {
            ...
            {
                logger.ErrorException("Error in TraktTVHelper.Scrobble: " + ex, ex);
                return 500;
            }
        }
        ...
        public static TraktV2ShowExtended GetShowInfoV2(string traktID, ref int traktCode)
        {
            ...
            {
                logger.ErrorException("Error in TraktTVHelper.GetShowInfo: " + ex, ex);
                return null;
            }
        ...
        public static void SaveExtendedShowInfoV2(TraktV2ShowExtended tvshow, List<TraktV2Season> seasons)
        {
            try
            {
                ...
                var repFanart = new Trakt_ImageFanartRepository();
                var repShows = new Trakt_ShowRepository();
                var show = repShows.GetByTraktSlug(tvshow.ids.slug);
                if (show == null)
                    show = new Trakt_Show();
                ...
                {
                    var season = repSeasons.GetByShowIDAndSeason(show.Trakt_ShowID, sea.number);
                    if (season == null)
                    ...
                    if (sea.images != null && sea.images.poster != null)
                    {
                        ...
                        {
                            var poster = repPosters.GetByShowIDAndSeason(show.Trakt_ShowID, season.Season);
                            if (poster == null)
                            {
                    ...
                    if (sea.episodes != null)
                    {
                        ...
                        {
                            var episode = repEpisodes.GetByShowIDSeasonAndEpisode(show.Trakt_ShowID, ep.season,
                                ep.number);
                            if (episode == null)
            ...
            {
                logger.ErrorException("Error in TraktTVHelper.SaveExtendedShowInfo: " + ex, ex);
            }
        }
        ...
        public static List<TraktV2Comment> GetShowCommentsV2(ISession session, int animeID)
        {
            ...
            try
            {
                ...
                var traktXRefs = repXrefTrakt.GetByAnimeID(session, animeID);
            ...
            {
                logger.ErrorException("Error in TraktTVHelper.GetShowComments: " + ex, ex);
            }
        ...
        {
            var friends = new List<TraktV2Follower>();
            ...
            try
            {
                ...
                var url = TraktURIs.GetUserFriends;
                ...
                foreach (var friend in resultFollowers)
                {
                    var traktFriend = repFriends.GetByUsername(friend.user.username);
                    if (traktFriend == null)
            ...
            {
                logger.ErrorException("Error in TraktTVHelper.GetFriends: " + ex, ex);
                return friends;
            }
