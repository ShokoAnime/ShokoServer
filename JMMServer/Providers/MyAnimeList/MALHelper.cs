...
namespace JMMServer.Providers.MyAnimeList
{
    public class MALHelper
    {
        ...
        public static anime SearchAnimesByTitle(string searchTitle)
        {
            ...
            var animes = GetEmptyAnimes();
            var entries = new List<animeEntry>();

            if (docSearchResult != null && docSearchResult["anime"] != null)
            {
                var searchItems = docSearchResult["anime"].GetElementsByTagName("entry");
        ...
        private static anime GetEmptyAnimes()
        {
            ...
            var animeEntries = new animeEntry[0];
        ...
        public static bool VerifyCredentials()
        {
            try
            {
                var username = ServerSettings.MAL_Username;
                var password = ServerSettings.MAL_Password;

                var url = "http://myanimelist.net/api/account/verify_credentials.xml";
                var webReq = (HttpWebRequest)WebRequest.Create(url);
                webReq.Timeout = 30 * 1000;
                ...
                var outputXML = Reader.ReadToEnd();
                ...
                var docVerifyCredentials = new XmlDocument();
                docVerifyCredentials.LoadXml(outputXML);

                var reader = new XmlNodeReader(docVerifyCredentials.DocumentElement);
            ...
            bool fromWebCache)
        {
            ...
            if (xrefTemp != null)
            {
                var msg = string.Format("Not using MAL link as this MAL ID ({0}) is already in use by {1}", malID,
                    xrefTemp.AnimeID);
                logger.Warn(msg);
                return;
            }
            ...
            var cmd = new CommandRequest_MALUpdatedWatchedStatus(animeID);
            cmd.Save();

            var req = new CommandRequest_WebCacheSendXRefAniDBMAL(xref.CrossRef_AniDB_MALID);
            req.Save();
        }
        ...
        {
            var repCrossRef = new CrossRef_AniDB_MALRepository();
            var xref = repCrossRef.GetByAnimeConstraint(animeID, epType, epNumber);
            if (xref == null) return;
            ...
            var req = new CommandRequest_WebCacheDeleteXRefAniDBMAL(animeID, epType, epNumber);
            req.Save();
        }
        ...
        public static void ScanForMatches()
        {
            ...
            foreach (var ser in allSeries)
            {
                ...
                var xrefs = anime.GetCrossRefMAL();
                if (xrefs == null || xrefs.Count == 0)
                {
                    logger.Trace(string.Format("Found anime without MAL association: {0} ({1})", anime.AnimeID,
                        anime.MainTitle));

                    var cmd = new CommandRequest_MALSearchAnime(ser.AniDB_ID, false);
                    cmd.Save();
                }
            }
        }
        ...
        public static myanimelist GetMALAnimeList()
        {
            try
            {
                ...
                var url = string.Format("http://myanimelist.net/malappinfo.php?u={0}&status=all&type=anime",
                    ServerSettings.MAL_Username);
                var malAnimeListXML = SendMALAuthenticatedRequest(url);
                malAnimeListXML = ReplaceEntityNamesByCharacter(malAnimeListXML);

                var serializer = new XmlSerializer(typeof(myanimelist));
                var docAnimeList = new XmlDocument();
                docAnimeList.LoadXml(malAnimeListXML);

                var reader = new XmlNodeReader(docAnimeList.DocumentElement);
                var obj = serializer.Deserialize(reader);
                var malAnimeList = (myanimelist)obj;
                return malAnimeList;
            }
            catch (Exception ex)
            {
        ...
        public static void UpdateMALSeries(AnimeSeries ser)
        {
            try
            {
                ...
                var animeListHashtable = new Hashtable();
                ...
                {
                    var malAnimeList = GetMALAnimeList();
                    if (malAnimeList != null && malAnimeList.anime != null)
                    {
                        for (var i = 0; i < malAnimeList.anime.Length; i++)
                        {
                ...
                // look for MAL Links
                var crossRefs = ser.GetAnime().GetCrossRefMAL();
                ...
                foreach (var xref in crossRefs)
                {
                    ...
                    var fanSubGroups = new List<string>();
                    ...
                    var lastWatchedEpNumber = 0;
                    var downloadedEps = 0;

                    foreach (var ep in eps)
                    {
                        var epNum = ep.AniDB_Episode.EpisodeNumber;
                            ...
                            epNum <= GetUpperEpisodeLimit(crossRefs, xref))
                        {
                            ...
                            var usrRecord = ep.GetUserRecord(user.JMMUserID);
                            if (usrRecord != null && usrRecord.WatchedDate.HasValue && epNum > lastWatchedEpNumber)
                            {
                                lastWatchedEpNumber = epNum;
                            }

                            var contracts = ep.GetVideoDetailedContracts(user.JMMUserID);
                    ...
                    var fanSubs = "";
                    foreach (var fgrp in fanSubGroups)
                    {
                    ...
                    // determine status
                    var status = 1; //watching
                    if (animeListHashtable.ContainsKey(malID))
                    {
                        var animeInList = (myanimelistAnime)animeListHashtable[malID];
                    ...
                    {
                        var res = UpdateAnime(malID, lastWatchedEpNumber, status, score, downloadedEps, fanSubs);
    ...
    internal class HtmlEntities
    {
        ...
        private static readonly string[] _entitiesList =
        {
            ...
            "\x00a4-curren", "\x00a5-yen", "\x00a6-brvbar", "\x00a7-sect", "\x00a8-uml", "\x00a9-copy", "\x00aa-ordf",
            "\x00ab-laquo",
            "\x00ac-not", "\x00ad-shy", "\x00ae-reg", "\x00af-macr", "\x00b0-deg", "\x00b1-plusmn", "\x00b2-sup2",
            "\x00b3-sup3", "\x00b4-acute", "\x00b5-micro", "\x00b6-para", "\x00b7-middot", "\x00b8-cedil", "\x00b9-sup1",
            "\x00ba-ordm", "\x00bb-raquo",
            "\x00bc-frac14", "\x00bd-frac12", "\x00be-frac34", "\x00bf-iquest", "\x00c0-Agrave", "\x00c1-Aacute",
            "\x00c2-Acirc", "\x00c3-Atilde", "\x00c4-Auml", "\x00c5-Aring", "\x00c6-AElig", "\x00c7-Ccedil",
            "\x00c8-Egrave", "\x00c9-Eacute", "\x00ca-Ecirc", "\x00cb-Euml",
            ...
            "Ÿ-Yuml", "ƒ-fnof", "ˆ-circ", "˜-tilde", "Α-Alpha", "Β-Beta", "Γ-Gamma", "Δ-Delta",
            ...
            "ψ-psi", "ω-omega", "ϑ-thetasym", "ϒ-upsih", "ϖ-piv",
            ...
            "⁄-frasl", "€-euro", "ℑ-image", "℘-weierp", "ℜ-real", "™-trade", "ℵ-alefsym",
        ...
        internal static char Lookup(string entity)
        {
            if (_entitiesLookupTable == null)
            {
                lock (_lookupLockObject)
                {
                    if (_entitiesLookupTable == null)
                    {
                        ...
                        foreach (var str in _entitiesList)
                        {
            ...
            var obj2 = _entitiesLookupTable[entity];
            if (obj2 != null)
            {
                return (char)obj2;
            }
    ...
    {
        private static readonly char[] s_entityEndingChars = { ';', '&' };
            ...
            if (s != null)
            {
                ...
                else
                {
                    ...
                    for (var i = 0; i < length; i++)
                    {
                        ...
                        if (ch == '&')
                        {
                            var num3 = s.IndexOfAny(s_entityEndingChars, i + 1);
                            if ((num3 > 0) && (s[num3] == ';'))
                            {
                                var entity = s.Substring(i + 1, num3 - i - 1);
