    ...
    public class Trakt_Episode
    {
        ...
        public string FullImagePath
        {
            get
            {
                ...
                var pos = EpisodeImage.IndexOf(@"episodes/");
                if (pos <= 0) return "";

                var posID = URL.IndexOf(@"show/");
                if (posID <= 0) return "";

                var posIDNext = URL.IndexOf(@"/", posID + 6);
                if (posIDNext <= 0) return "";

                var traktID = URL.Substring(posID + 5, posIDNext - posID - 5);
                traktID = traktID.Replace("/", @"\");

                var imageName = EpisodeImage.Substring(pos + 9, EpisodeImage.Length - pos - 9);
                imageName = imageName.Replace("/", @"\");

                var relativePath = Path.Combine("episodes", traktID);
        ...
        {
            var contract = new Contract_Trakt_Episode();
