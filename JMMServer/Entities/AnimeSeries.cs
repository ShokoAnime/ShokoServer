...
namespace JMMServer.Entities
{
    public class AnimeSeries
    {
        ...
        public AnimeGroup TopLevelAnimeGroup
        {
            get
            {
                AnimeGroup parentGroup = RepoFactory.AnimeGroup.GetByID(this.AnimeGroupID);
        ...
        public List<AnimeGroup> AllGroupsAbove
        {
            get
            {
                ...
                try
                {
                    ...
                    {
                        AnimeGroup grp = RepoFactory.AnimeGroup.GetByID(groupID.Value);
                        if (grp != null)
                        {
                            grps.Add(grp);
        ...
        public static Dictionary<int, HashSet<GroupFilterConditionType>> BatchUpdateContracts(ISessionWrapper session,
            IReadOnlyCollection<AnimeSeries> seriesBatch, bool onlyStats = false)
        {
            if (session == null)
            ...
            {
                Contract_AnimeSeries contract = series.Contract?.DeepClone();
                ...
                {
                    // AniDB contract
                    AniDB_Anime animeRec = series.GetAnime();
                    ...
                    {
                        contract.CrossRefAniDBMovieDB = movieDbInfo.Item1.ToContract();
        ...
        public void UpdateStats(bool watchedStats, bool missingEpsStats, bool updateAllGroupsAbove)
        {
            ...
            if (watchedStats)
            {
                foreach (JMMUser juser in allUsers)
                {
                    ...
                    List<AnimeEpisode_User> epUserRecords = RepoFactory.AnimeEpisode_User.GetByUserID(juser.JMMUserID);
