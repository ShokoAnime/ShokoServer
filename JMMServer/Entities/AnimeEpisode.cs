    ...
    public class AnimeEpisode
    {
        ...
        public AniDB_Episode AniDB_Episode
        {
            get
            {
                ...
                return repEps.GetByEpisodeID(AniDB_EpisodeID);
            }
        }
        ...
        {
            AniDB_EpisodeID = anidbEp.EpisodeID;
        ...
        public AnimeEpisode_User GetUserRecord(ISession session, int userID)
        {
            ...
            return repEpUser.GetByUserIDAndEpisodeID(session, userID, AnimeEpisodeID);
        }
        ...
        public Contract_AnimeEpisode ToContract(ISession session, bool getFileCount, int userID,
            AnimeSeries_User seruser)
        {
            var contract = new Contract_AnimeEpisode();
            contract.AniDB_EpisodeID = AniDB_EpisodeID;
            contract.AnimeEpisodeID = AnimeEpisodeID;
            contract.AnimeSeriesID = AnimeSeriesID;
            contract.DateTimeUpdated = DateTimeUpdated;

            var epuser = GetUserRecord(session, userID);
            ...
            if (getFileCount)
                contract.LocalFileCount = GetVideoLocals(session).Count;
        ...
        public AnimeSeries GetAnimeSeries(ISession session)
        {
            ...
            return repSeries.GetByID(session, AnimeSeriesID);
        }
        ...
        public List<Contract_VideoDetailed> GetVideoDetailedContracts(int userID)
        {
            ...
            var contracts = new List<Contract_VideoDetailed>();

            // get all the cross refs
            foreach (var xref in FileCrossRefs)
            {
                ...
                var vid = repVids.GetByHash(xref.Hash);
                if (vid != null)
                {
                    ...
                    var vidUser = vid.GetUserRecord(userID);
                    ...
                    // Import Folder
                    var ns = vid.ImportFolder; // to prevent multiple db calls
                    ...
                    // video info
                    var vi = vid.VideoInfo; // to prevent multiple db calls
                    ...
                    // AniDB File
                    var anifile = vid.GetAniDBFile(); // to prevent multiple db calls
                    if (anifile != null)
                    {
                    ...
                    var relGroup = vid.ReleaseGroup; // to prevent multiple db calls
                    if (relGroup != null)
