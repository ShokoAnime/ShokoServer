...
namespace JMMServer.Entities
{
    public class AnimeGroup
    {
        ...
        public static int RelationCompare(AniDB_Anime_Relation thisGroup, AniDB_Anime_Relation otherGroup)
        {
            // if this returns -1 then thisGroup is first
            if (thisGroup.RelationType.ToLower().Trim().Equals("prequel")) return -1;
            if (thisGroup.RelationType.ToLower().Trim().Equals("sequel")) return 1;
            if (otherGroup.RelationType.ToLower().Trim().Equals("sequel")) return -1;
            if (otherGroup.RelationType.ToLower().Trim().Equals("prequel")) return 1;
            if (thisGroup.AnimeID < otherGroup.AnimeID) return -1;
            return 1;
        }

        public static List<AnimeGroup> GetRelatedGroupsFromAnimeID(int animeid, Boolean forceRecursive=false)
        {
            using (var session = JMMService.SessionFactory.OpenSession())
            {
        ...
        public static List<AnimeGroup> GetRelatedGroupsFromAnimeID(ISession session, int animeid, Boolean forceRecursive=false)
        {
            ...
            relations.Sort(RelationCompare);
