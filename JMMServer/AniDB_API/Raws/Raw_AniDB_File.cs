...
namespace AniDBAPI
{
    ...
    {
        public static readonly int LastVersion = 2;
        // default constructor
        ...
        public Raw_AniDB_File(string sRecMessage)
        {
            ...
            var sDetails = sRecMessage.Substring(9).Split('|');
            ...
            var state = int.Parse(sDetails[7].Trim());
            if (state == 0 || state == 1)
                FileVersion = 1;
            else
            {
                var eState = (AniDBFileState)state;
            ...
            // mylist values
            var mlState = sDetails[27].Trim();
            var mlFileState = sDetails[28].Trim();
            var mlViewed = sDetails[29].Trim();
            var mlViewDate = sDetails[30].Trim();
            var mlStorage = sDetails[31].Trim();
            var mlSource = sDetails[32].Trim();
            var mlOther = sDetails[33].Trim();
