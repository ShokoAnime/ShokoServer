...
namespace AniDBAPI
{
    ...
    public class Raw_AniDB_Character : XMLBase
    {
        ...
        public void ProcessFromHTTPResult(XmlNode node, int anid)
        {
            ...
            CharName = AniDBHTTPHelper.TryGetProperty(node, "name");
            CharDescription = AniDBHTTPHelper.TryGetProperty(node, "description");
            EpisodeListRaw = AniDBHTTPHelper.TryGetProperty(node, "episodes");
            ...
            foreach (XmlNode nodeChild in node.ChildNodes)
            {
                ...
                {
                    var seiyuu = new Raw_AniDB_Seiyuu();

                    if (nodeChild.Attributes["id"] != null)
                    {
                        var creatorid = nodeChild.Attributes["id"].Value;
