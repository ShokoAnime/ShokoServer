...
namespace AniDBAPI
{
    ...
    {
        public static readonly int LastVersion = 1;
        ...
        public Raw_AniDB_Anime(string sRecMessage)
        {
            ...
            var sDetails = sRecMessage.Substring(10).Split('|');
            ...
            var airDateSeconds = int.Parse(AniDBAPILib.ProcessAniDBString(sDetails[16]));
            var endDateSeconds = int.Parse(AniDBAPILib.ProcessAniDBString(sDetails[17]));
            ...
            foreach (var rid in ReviewIDList)
            {
        ...
        public List<int> ReviewIDList
        {
            get
            {
                var reviewIDList = new List<int>();

                try
                {
                    if (ReviewIDListRAW.Length > 0)
                    {
                        var reviews = ReviewIDListRAW.Split(',');
