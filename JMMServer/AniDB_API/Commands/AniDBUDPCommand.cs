            ...
            ref IPEndPoint remoteIpEndPoint, string sessionID, Encoding enc)
        {
            ...
            if (commandType != enAniDBCommandType.Ping)
            {
                ...
                else
                {
                    ...
                    changeencoding = enc;
                    var encod = changeencoding.EncodingName;
            ...
            do
            {
                if (part > 0)
                {
                    mcommandText = mcommandText.Replace("part=" + (part - 1), "part=" + part);
                    Thread.Sleep(2300);
                }
                if (commandType != enAniDBCommandType.Login)
                {
                    var msg = string.Format("UDP_COMMAND: {0}", mcommandText);
                    JMMService.LogToSystem(Constants.DBLogType.APIAniDBUDP, msg);
                }
                bool repeatcmd;
                int received;
                var byReceivedAdd = new byte[2000]; // max length should actually be 1400
                do
                {
                    repeatcmd = false;
                    // Send Message
                    var SendByteAdd = Encoding.GetBytes(mcommandText.ToCharArray());
                ...
                if (!errorOccurred)
                {
                        ...
                        commandType == enAniDBCommandType.GetReview)
                    {
                        ...
                        part++;
                        var sp1 = decodedstring.Split(new[] { '\n' }, StringSplitOptions.RemoveEmptyEntries);
            ...
            else
            {
                ...
                if (i != 2)
                {
                    ...
                    var msg = string.Format("UDP_RESPONSE in {0} ms - {1} ", ts.TotalMilliseconds, socketResponse);
                    JMMService.LogToSystem(Constants.DBLogType.APIAniDBUDP, msg);
                }
                else
                {
                    ...
                    var ts = DateTime.Now - start;
                    var msg = string.Format("UDP_RESPONSE_TRUNC in {0}ms - {1} ", ts.TotalMilliseconds, socketResponse);
                    JMMService.LogToSystem(Constants.DBLogType.APIAniDBUDP, msg);
                }
            }
            var val = 0;
