...
namespace AniDBAPI
{
    public class AniDBHTTPHelper
    {
        ...
        public static void GetAnime(int animeID, bool createSeriesRecord)
        {
            try
            {
                var uri = string.Format(AnimeURL, animeID);
        ...
        public static XmlDocument GetAnimeXMLFromAPI(int animeID, ref string rawXML)
        {
            try
            {
                ...
                var msg = string.Format(Resources.AniDB_GettingAnimeXML, animeID);
                ...
                msg = string.Format(Resources.AniDB_GotAnimeXML, animeID, ts.TotalMilliseconds, content);
                JMMService.LogToSystem(Constants.DBLogType.APIAniDBHTTP, msg);
        ...
        public static XmlDocument GetMyListXMLFromAPI(string username, string password, ref string rawXML)
        {
            try
            {
                ...
                var uri = string.Format(MyListURL, username, password);
                rawXML = APIUtils.DownloadWebPage(uri);
        ...
        public static XmlDocument GetVotesXMLFromAPI(string username, string password, ref string rawXML)
        {
            try
            {
                var uri = string.Format(VotesURL, username, password);
                rawXML = APIUtils.DownloadWebPage(uri);
        ...
        public static Raw_AniDB_Anime ProcessAnimeDetails(XmlDocument docAnime, int animeID)
        {
            ...
            var anime = new Raw_AniDB_Anime();
            anime.AnimeID = animeID;

            // check if there is any data
            try
            {
                var id = docAnime["anime"].Attributes["id"].Value;
            }
            catch (Exception ex)
            {
            ...
            var convertedAirDate = Utils.GetAniDBDateAsSeconds(TryGetProperty(docAnime, "anime", "startdate"), true);
            var convertedEndDate = Utils.GetAniDBDateAsSeconds(TryGetProperty(docAnime, "anime", "enddate"), false);
                ...
                if (raItems != null)
                {
                    ...
                    foreach (XmlNode node in raItems)
                    {
                        try
                        {
                            ...
                            var relType = ConvertReltTypeTextToEnum(TryGetAttribute(node, "type"));
        ...
        {
            var xmlResult = "";
            var docAnime = GetAnimeXMLFromAPI(animeID, ref xmlResult);
            if (docAnime == null)
                return null;
        ...
        {
            var tags = new List<Raw_AniDB_Tag>();
        ...
        {
            var chars = new List<Raw_AniDB_Character>();
                ...
                if (docAnime["anime"]["characters"] != null)
                {
                    ...
                    if (charItems != null)
                    {
                        foreach (XmlNode node in charItems)
                        {
                            try
                            {
                                var chr = new Raw_AniDB_Character();
        ...
        {
            var titles = new List<Raw_AniDB_Anime_Title>();
                ...
                if (docAnime["anime"]["titles"] != null)
                {
                    ...
                    if (titleItems != null)
                    {
                        foreach (XmlNode node in titleItems)
                        {
                            try
                            {
                                var animeTitle = new Raw_AniDB_Anime_Title();
            ...
            {
                var msg = string.Format("Error in AniDBHTTPHelper.ProcessTitles: {0} - {1}", animeID, ex);
        ...
        {
            var rels = new List<Raw_AniDB_RelatedAnime>();
                ...
                if (docAnime["anime"]["relatedanime"] != null)
                {
                    ...
                    if (relItems != null)
                    {
                        foreach (XmlNode node in relItems)
                        {
                            try
                            {
                                var rel = new Raw_AniDB_RelatedAnime();
        ...
        {
            var rels = new List<Raw_AniDB_SimilarAnime>();
                ...
                if (docAnime["anime"]["similaranime"] != null)
                {
                    ...
                    if (simItems != null)
                    {
                        foreach (XmlNode node in simItems)
                        {
                            try
                            {
                                var sim = new Raw_AniDB_SimilarAnime();
        ...
        {
            var recs = new List<Raw_AniDB_Recommendation>();

            try
            {
                if (docAnime["anime"]["recommendations"] != null)
                {
                    var recItems = docAnime["anime"]["recommendations"].GetElementsByTagName("recommendation");
                    if (recItems != null)
                    {
                        foreach (XmlNode node in recItems)
                        {
                            try
                            {
                                var rec = new Raw_AniDB_Recommendation();
        ...
        {
            var eps = new List<Raw_AniDB_Episode>();
        ...
        {
            var mylistentries = new List<Raw_AniDB_MyListFile>();

            try
            {
                if (docAnime != null && docAnime["mylist"] != null)
                {
                    var myitems = docAnime["mylist"].GetElementsByTagName("mylistitem");

                    foreach (XmlNode node in myitems)
                    {
                        try
                        {
                            var mylistitem = new Raw_AniDB_MyListFile();
                            mylistitem.ProcessHTTPSource(node);
                            mylistentries.Add(mylistitem);
                        }
                        catch (Exception ex)
                        {
                            logger.ErrorException("Error in ProcessEpisodes: {0}" + ex, ex);
                        }
                    }
                }
            }
        ...
        {
            var myvotes = new List<Raw_AniDB_Vote_HTTP>();
            ...
            if (docAnime != null && docAnime["votes"] != null)
            {
                ...
                try
                {
                    ...
                    foreach (XmlNode node in myitems)
                    {
                        var thisVote = new Raw_AniDB_Vote_HTTP();
        ...
        public static string TryGetAttribute(XmlNode parentnode, string nodeName, string attName)
        {
            try
            {
                var prop = parentnode[nodeName].Attributes[attName].Value;
                return prop;
            }
            catch
            {
        ...
        public static string TryGetAttribute(XmlNode node, string attName)
        {
            try
            {
                var prop = node.Attributes[attName].Value;
                return prop;
            }
            catch
            {
