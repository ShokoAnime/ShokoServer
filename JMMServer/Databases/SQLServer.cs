...
namespace JMMServer.Databases
{
    public class SQLServer
    {
        ...
        {
            var count = 0;
            var cmd = string.Format("Select count(*) from sysdatabases where name = '{0}'", ServerSettings.DatabaseName);
            using (
                var tmpConn =
        ...
        public static ArrayList GetData(string sql)
        {
                        ...
                        ServerSettings.DatabaseUsername, ServerSettings.DatabasePassword, ServerSettings.DatabaseName)))
            {
                ...
                using (var command = new SqlCommand(sql, tmpConn))
                {
                    tmpConn.Open();
        ...
        public static void CreateDatabaseOld()
        {
            ...
            var dataPath = GetDatabasePath(ServerSettings.DatabaseServer);
            ...
            {
                using (var command = new SqlCommand(sb.ToString(), tmpConn))
                {
                    tmpConn.Open();
        ...
        public static void CreateDatabase()
        {
            ...
            var conn = new ServerConnection(ServerSettings.DatabaseServer, ServerSettings.DatabaseUsername,
                ServerSettings.DatabasePassword);
            var srv = new Server(conn);

            var db = new Database(srv, ServerSettings.DatabaseName);
            db.Create();
        }
        ...
        {
            var instName = GetInstanceNameFromServerName(serverName).Trim().ToUpper();


            //
            using (var sqlServerKey = Registry.LocalMachine.OpenSubKey(registryPoint))
            {
                foreach (var subKeyName in sqlServerKey.GetSubKeyNames())
                {
                    if (subKeyName.StartsWith("MSSQL"))
                    {
                        using (var instanceKey = sqlServerKey.OpenSubKey(subKeyName))
                        {
                            var val = instanceKey.GetValue("");
                            if (val != null)
                            {
                                var instanceName = val.ToString().Trim().ToUpper();

                                if (instanceName == instName) //say
                                {
                                    var path = instanceKey.OpenSubKey(@"Setup").GetValue("SQLDataRoot").ToString();
        ...
        public static string GetInstanceNameFromServerName(string servername)
        {
            ...
            var instancename = servername.Substring(pos + 1, servername.Length - pos - 1);
        ...
        public static void UpdateSchema()
        {
            ...
            {
                logger.ErrorException("Error updating schema: " + ex, ex);
            }
        }
        ...
        private static void UpdateSchema_002(int currentVersionNumber)
        {
                        ...
                        ServerSettings.DatabaseUsername, ServerSettings.DatabasePassword, ServerSettings.DatabaseName)))
            {
                ...
                {
                    using (var command = new SqlCommand(cmdTable, tmpConn))
                    {
                        command.ExecuteNonQuery();
                    }
                }
        ...
        {
            var count = 0;
            var cmd = "Select count(*) from sysobjects where name = \'Versions\'";
            using (
                var tmpConn =
                    new SqlConnection(string.Format("Server={0};User ID={1};Password={2};database={3}",
                        ServerSettings.DatabaseServer,
            ...
            var ver1 = new Versions();
            ver1.VersionType = Constants.DatabaseTypeKey;
            ver1.VersionValue = "1";

            var repVer = new VersionsRepository();
            repVer.Save(ver1);
        }
