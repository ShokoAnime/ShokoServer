			...
			try
			{
				...
				using (var session = JMMService.SessionFactory.OpenSession())
				{
					...
					foreach (CrossRef_AniDB_Trakt xrefTrakt in xrefsTrakt)
					{
						...
						CrossRef_AniDB_TraktV2 temp = repCrossRefTraktNew.GetByTraktID(xrefTrakt.TraktID, 0, 1, xrefTrakt.AnimeID, (int)enEpisodeType.Special, 1);
			...
			try
			{
				...
				using (var session = JMMService.SessionFactory.OpenSession())
				{
					...
					foreach (CrossRef_AniDB_TvDB xrefTvDB in xrefsTvDB)
					{
						...
						//make sure we are not doubling up
						CrossRef_AniDB_TvDBV2 temp = repCrossRefTvDBNew.GetByTvDBID(xrefTvDB.TvDBID, 0, 1, xrefTvDB.AnimeID, (int)enEpisodeType.Special, 1);
						if (temp != null) continue;
