...
namespace JMMServer.Databases
{
    public class SQLite
    {
        ...
        public static string GetDatabasePath()
        {
            ...
            var dbPath = Path.Combine(appPath, "SQLite");
            return dbPath;
        }

        public static string GetDatabaseFilePath()
        {
            var dbName = Path.Combine(GetDatabasePath(), DefaultDBName);
            return dbName;
        }
        ...
        public static ArrayList GetData(string sql)
        {
            ...
            try
            {
                ...
                var reader = sqCommand.ExecuteReader();
        ...
        private static void UpdateSchema_002(int currentVersionNumber)
        {
            ...
            {
                var sqCommand = new SQLiteCommand(cmdTable);
                sqCommand.Connection = myConn;
        ...
        public static void CreateInitialSchema()
        {
            ...
            var cmd = "SELECT count(*) as NumTables FROM sqlite_master WHERE name=\'Versions\'";
            var sqCommandCheck = new SQLiteCommand(cmd);
            sqCommandCheck.Connection = myConn;
            var count = long.Parse(sqCommandCheck.ExecuteScalar().ToString());
