    ...
    public class CommandRequest_HashFile : CommandRequestImplementation, ICommandRequest
    {
        ...
        public string PrettyDescription
        {
            get
            {
                ...
                return string.Format(Resources.Command_HashingFile, FileName);
            }
        }
        ...
        private VideoLocal ProcessFile_LocalInfo()
        {
            ...
            VideoLocal vlocal = null;
            var repVidLocal = new VideoLocalRepository();
            var repFNHash = new FileNameHashRepository();

            var vidLocals = repVidLocal.GetByFilePathAndShareID(filePath, nshareID);
            ...
            if (string.IsNullOrEmpty(vlocal.Hash) || ForceHash)
            {
                ...
                if (!ForceHash)
                {
                    ...
                    var crossRefs = repCrossRefs.GetByFileNameAndSize(Path.GetFileName(vlocal.FilePath), vlocal.FileSize);
                ...
                {
                    var fnhashes = repFNHash.GetByFileNameAndSize(Path.GetFileName(vlocal.FilePath), vlocal.FileSize);
                ...
                var vidTemp = repVidLocal.GetByHash(vlocal.Hash);
                if (vidTemp != null)
                {
                    ...
                    if (vidTemp.VideoLocalID != vlocal.VideoLocalID)
                    {
                        ...
                        var repDups = new DuplicateFileRepository();
                        var dupFiles = repDups.GetByFilePathsAndImportFolder(vlocal.FilePath, vidTemp.FilePath,
                ...
                var fnhashes2 = repFNHash.GetByFileNameAndSize(Path.GetFileName(vlocal.FilePath), vlocal.FileSize);
                if (fnhashes2 != null && fnhashes2.Count > 1)
                {
                    // if we have more than one record it probably means there is some sort of corruption
                    // lets delete the local records
                    foreach (var fnh in fnhashes2)
                    {
                        repFNHash.Delete(fnh.FileNameHashID);
                    }
                }
            ...
            var cr_procfile = new CommandRequest_ProcessFile(vlocal.VideoLocalID, false);
            cr_procfile.Save();
        ...
        {
            CommandID = string.Format("CommandRequest_HashFile_{0}", FileName);
        }
