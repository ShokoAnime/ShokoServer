...
namespace BinaryNorthwest
{
    ...
    public static class Sorting
    {
        ...
        /// <param name="sortBy">
        ///     Criteria describing the field or property name used in sorting the list
        ...
        public static void SortInPlace<T>(List<T> ListToBeSorted, SortPropOrFieldAndDirection sortBy) where T : class
        {
            try
            {
                ...
                var compare = sortBy.GetComparer<T>();
                lock (ListToBeSorted)
                {
            ...
            catch (Exception ex)
            {
                throw new Exception("Error trying to sort list of " + typeof(T).Name + " using " +
                                    (sortBy.NameIsPropertyName ? "property " : "field ") + sortBy.sPropertyOrFieldName,
                    ex);
            }
        }
        ...
        /// <returns>
        ///     A new list containing all elements of ListToBeSorted, sorted by the criteria
        ///     specified in rgSortBy.
        /// </returns>
        public static List<T> MultiSort<T>(List<T> ListToBeSorted, List<SortPropOrFieldAndDirection> rgSortBy)
            where T : class
        {
            List<T> results;
            ...
            if (rgSortBy.Count == 1)
            {
                ...
                SortInPlace(results, rgSortBy[0]);
                return results;
            }

            try
            {
                var rgCopies = new List<List<T>>(1);
                rgCopies.Add(results);
                var sortByCount = rgSortBy.Count;
                ...
                for (var i = 0; i < sortByCount; i++)
                {
                    var sortBy = rgSortBy[i];
                    if (string.IsNullOrEmpty(sortBy.sPropertyOrFieldName))
                        throw new Exception(
                            "MultiSort parameter rgSortBy was passed an empty field name in rgSortBy[" + i + "]"
                            );
                    ...
                    if (i < sortByCount - 1)
                    {
                        ...
                        var rgNewCopies = new List<List<T>>(rgCopies.Count * 8);

                        for (var n = 0; n < rgCopies.Count; n++)
                        {
                            var rgList = rgCopies[n];
                            ...
                            var rgSublist = new List<T>(rgList.Count / 32);
                ...
                results.Clear();
                foreach (var rgList in rgCopies)
                {
        ...
        public static SortType GetSortTypeEnumForType(Type t)
        {
            ...
            if (t == typeof(DateTime)) return SortType.eDateTime;
            if (t == typeof(int) || t == typeof(short)) return SortType.eInteger;
            if (t == typeof(long)) return SortType.eLong;
            if (t == typeof(bool)) return SortType.eByte;
            if (t == typeof(double) || t == typeof(float)) return SortType.eDoubleOrFloat;
            return SortType.eString;
        }
    }
}