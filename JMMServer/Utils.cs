...
namespace JMMServer
{
    public static class Utils
    {
        ...
        {
            "SOURCE", "TAKEN", "FROM", "HTTP", "ANN", "ANIMENFO", "ANIDB",
            "ANIMESUKI"
        };
        ...
        /// <summary>
        ///     Setup system with needed network settings for JMMServer operation. Will invoke an escalation prompt to user. If
        ///     changing port numbers please give new and old port.
        /// </summary>
            ...
            string oldFilePort = null)
        {
            ...
            try
            {
                ...
                {
                    BatchFileStream.WriteLine(@"netsh http delete urlacl url=http://+:{0}/JMMServerImage", 8111);
                    BatchFileStream.WriteLine(@"netsh http delete urlacl url=http://+:{0}/JMMServerBinary", 8111);
                    BatchFileStream.WriteLine(@"netsh http delete urlacl url=http://+:{0}/JMMServerMetro", 8111);
                    BatchFileStream.WriteLine(@"netsh http delete urlacl url=http://+:{0}/JMMServerMetroImage", 8111);
                    BatchFileStream.WriteLine(@"netsh http delete urlacl url=http://+:{0}/JMMServerPlex", 8111);
                    BatchFileStream.WriteLine(@"netsh http delete urlacl url=http://+:{0}/JMMServerKodi", 8111);
                    BatchFileStream.WriteLine(@"netsh http delete urlacl url=http://+:{0}/JMMServerREST", 8111);
                    BatchFileStream.WriteLine(@"netsh http delete urlacl url=http://+:{0}/JMMServerStreaming", 8111);
                ...
                {
                    BatchFileStream.WriteLine(@"netsh http delete urlacl url=http://+:{0}/JMMFilePort", 8112);
                    BatchFileStream.WriteLine(
                ...
                if (!string.IsNullOrEmpty(Port))
                {
                    ...
                    BatchFileStream.WriteLine(@"netsh http add urlacl url=http://+:{0}/JMMServerImage user=everyone",
                        Port);
                    BatchFileStream.WriteLine(@"netsh http add urlacl url=http://+:{0}/JMMServerBinary user=everyone",
                        Port);
                    BatchFileStream.WriteLine(@"netsh http add urlacl url=http://+:{0}/JMMServerMetro user=everyone",
                        Port);
                    BatchFileStream.WriteLine(
                        @"netsh http add urlacl url=http://+:{0}/JMMServerMetroImage user=everyone", Port);
                    BatchFileStream.WriteLine(@"netsh http add urlacl url=http://+:{0}/JMMServerPlex user=everyone",
                        Port);
                    BatchFileStream.WriteLine(@"netsh http add urlacl url=http://+:{0}/JMMServerKodi user=everyone",
                        Port);
                    BatchFileStream.WriteLine(@"netsh http add urlacl url=http://+:{0}/JMMServerREST user=everyone",
                        Port);
                    BatchFileStream.WriteLine(
                ...
                if (!string.IsNullOrEmpty(FilePort))
                {
                    ...
                    BatchFileStream.WriteLine(@"netsh http add urlacl url=http://+:{0}/JMMFilePort user=everyone",
                        FilePort);
                }

                if (!string.IsNullOrEmpty(oldPort))
                {
                    BatchFileStream.WriteLine(@"netsh http delete urlacl url=http://+:{0}/JMMServerImage", oldPort);
                    BatchFileStream.WriteLine(@"netsh http delete urlacl url=http://+:{0}/JMMServerBinary", oldPort);
                    BatchFileStream.WriteLine(@"netsh http delete urlacl url=http://+:{0}/JMMServerMetro", oldPort);
                    BatchFileStream.WriteLine(@"netsh http delete urlacl url=http://+:{0}/JMMServerMetroImage", oldPort);
                    BatchFileStream.WriteLine(@"netsh http delete urlacl url=http://+:{0}/JMMServerPlex", oldPort);
                    BatchFileStream.WriteLine(@"netsh http delete urlacl url=http://+:{0}/JMMServerKodi", oldPort);
                    BatchFileStream.WriteLine(@"netsh http delete urlacl url=http://+:{0}/JMMServerREST", oldPort);
                    BatchFileStream.WriteLine(@"netsh http delete urlacl url=http://+:{0}/JMMServerStreaming", oldPort);
                }
                if (!string.IsNullOrEmpty(oldFilePort))
                    BatchFileStream.WriteLine(@"netsh http delete urlacl url=http://+:{0}/JMMFilePort", oldFilePort);
        ...
        public static string DownloadWebPage(string url)
        {
            ...
            {
                var msg = "---------- ERROR IN DOWNLOAD WEB PAGE ---------" + Environment.NewLine +
        ...
        public static int GetAniDBDateAsSeconds(string dateXML, bool isStartDate)
        {
            ...
            var year = dateXML.Trim().Substring(0, 4);
            ...
            else
            {
                ...
                else
                {
                    ...
                    var numberOfDays = DateTime.DaysInMonth(int.Parse(year), int.Parse(month));
            ...
            var actualDate = new DateTime(int.Parse(year), int.Parse(month), int.Parse(day), 0, 0, 0);
        ...
        {
            var sp = date.Subtract(new DateTime(1970, 1, 1, 0, 0, 0));
        ...
        public static string ReparseDescription(string description)
        {
            ...
            {
                var a = vup.IndexOf("[URL");
                if (a >= 0)
                {
                    var b = vup.IndexOf("]", a + 1);
                    if (b >= 0)
                    {
        ...
        {
            // get %5 differentials
            var widthLower = width * 0.95;
            var widthUpper = width * 1.05;

            var heightLower = height * 0.95;
            var heightUpper = height * 1.05;
        ...
        {
            var vidWidth = GetVideoWidth(videoResolution);
            var score = 0;
        ...
        public static void GetFilesForImportFolder(string sDir, ref List<string> fileList)
        {
            try
            {
                ...
                foreach (var d in Directory.GetDirectories(sDir))
                {
                    ...
                    var isSystem = (di.Attributes & FileAttributes.System) == FileAttributes.System;
                    if (isSystem)
                        continue;
            ...
            ref string errorMessage, ref string streamingUri)
        {
            try
            {
                ...
                var subtitleTranscoder = "soverlay";
                //string muxerOptions = ":standard{access=file,mux=ts,dst=#OUT#}";
                var muxerOptions = ":standard{access=file,mux=ts,dst=8088}";

                var sout = "#transcode{" + encoderOptions + "," + subtitleTranscoder;
                ...
                var vlcStartTemplate = @" -v {0} --ffmpeg-hw --sout-ffmpeg-strict=-2 --sout={1}";

                var vlcStop = @"/F /IM vlc.exe";
                //string vlcStart = string.Format(vlcStartTemplate, fileName, vidBitRate, fps, resWidth, audioBitRate, audioSamplerate, port);
                var vlcStart = string.Format(vlcStartTemplate, fileName, sout);
