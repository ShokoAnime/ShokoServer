...
namespace JMMServer.Repositories
{
	public class AnimeEpisode_UserRepository
	{
        ...
        public static void InitCache()
        {
            ...
            int cnt = 0;
            List<AnimeEpisode_User> sers = Cache.Values.Where(a => a.ContractVersion < AnimeEpisode_User.CONTRACT_VERSION).ToList();
            int max = sers.Count;
		...
		{
		    return GetByUserID(userid).Where(a => a.WatchedCount > 0).OrderByDescending(a => a.WatchedDate).Take(maxresults).ToList();
		}
