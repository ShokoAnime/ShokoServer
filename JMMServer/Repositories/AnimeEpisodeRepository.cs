...
namespace JMMServer.Repositories
{
    public class AnimeEpisodeRepository
    {
        ...
        public List<AnimeEpisode> GetEpisodesWithMultipleFiles(bool ignoreVariations)
        {
            using (var session = JMMService.SessionFactory.OpenSession())
            {
                    ...
                    .Equals(Constants.DatabaseType.MySQL, StringComparison.InvariantCultureIgnoreCase))
                {
                    ...
                    var sql = "Select x.AnimeEpisodeID " +
                              "FROM AnimeEpisode x " +
                              "INNER JOIN  " +
                    ...
                    {
                        var animeEpisodeID = int.Parse(res[0].ToString());
                        var ep = session.Get<AnimeEpisode>(animeEpisodeID);
                        if (ep != null)
                            epList.Add(ep);
                    }
                ...
                {
                    var sql = "FROM AnimeEpisode x WHERE x.AniDB_EpisodeID IN " +
        ...
        public void Delete(int id)
        {
            ...
            {
                // delete user records
                var repUsers = new AnimeEpisode_UserRepository();
                foreach (var epuser in repUsers.GetByEpisodeID(id))
