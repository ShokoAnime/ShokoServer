...
namespace JMMServer.WebCache
{
	public class XMLService
	{
		...
		public static void Send_CrossRef_AniDB_TvDB(CrossRef_AniDB_TvDB data, string seriesName)
		{
			if (!ServerSettings.WebCache_XRefFileEpisode_Send) return;


			string uri = string.Format("http://{0}/AddCrossRef_AniDB_TvDB.aspx", ServerSettings.WebCache_Address);
			AddCrossRef_AniDB_TvDBRequest fhr = new AddCrossRef_AniDB_TvDBRequest(data, seriesName);
			string xml = fhr.ToXML();

			SendData(uri, xml);
		}

		public static void Delete_CrossRef_AniDB_TvDB(int animeID)
		{
			if (!ServerSettings.WebCache_XRefFileEpisode_Send) return;
			if (ServerSettings.WebCache_Anonymous) return;

			string uri = string.Format("http://{0}/DeleteCrossRef_AniDB_TvDB.aspx", ServerSettings.WebCache_Address);
			DeleteCrossRef_AniDB_TvDBRequest req = new DeleteCrossRef_AniDB_TvDBRequest(animeID);
			string xml = req.ToXML();
		...
		public static void Delete_CrossRef_AniDB_TvDB_All(int tvDBID)
		{
			if (!ServerSettings.WebCache_XRefFileEpisode_Send) return;
			if (ServerSettings.WebCache_Anonymous) return;

			string uri = string.Format("http://{0}/DeleteCrossRef_AniDBTvDBAll.aspx", ServerSettings.WebCache_Address);
			DeleteCrossRef_AniDBTvDBAll_Request req = new DeleteCrossRef_AniDBTvDBAll_Request(tvDBID);
			string xml = req.ToXML();
		...
		public static CrossRef_AniDB_TvDBResult Get_CrossRef_AniDB_TvDB(int animeID)
		{
			...
			try
			{
				...
				string sTvDBID = TryGetProperty(docFile, "CrossRef_AniDB_TvDBResult", "TvDBID");
				string sTvDBSeasonNumber = TryGetProperty(docFile, "CrossRef_AniDB_TvDBResult", "TvDBSeasonNumber");
				string sAdminApproved = TryGetProperty(docFile, "CrossRef_AniDB_TvDBResult", "AdminApproved");
				string seriesName = TryGetProperty(docFile, "CrossRef_AniDB_TvDBResult", "SeriesName");


				int TvDBID = 0;
				int.TryParse(sTvDBID, out TvDBID);

				int TvDBSeasonNumber = 0;
				int.TryParse(sTvDBSeasonNumber, out TvDBSeasonNumber);
				...
				CrossRef_AniDB_TvDBResult result = new CrossRef_AniDB_TvDBResult();
				result.AnimeID = animeID;
				result.TvDBID = TvDBID;
				result.TvDBSeasonNumber = TvDBSeasonNumber;
