                        ...
                        StatsCache.Instance.StatUserGroupFilter[user.JMMUserID].ContainsKey(gf.GroupFilterID))
                    {
                        ...
                        foreach (Contract_AnimeGroup grp in comboGroups)
                        {
                            ...
                            foreach (AnimeSeries ser in repSeries.GetByGroupID(session, grp.AnimeGroupID))
                            {
                                ...
                                Contract_AnimeEpisode ep = imp.GetNextUnwatchedEpisode(session, ser.AnimeSeriesID,
                                    jmmuserID);
                                if (ep != null)
                                {
                                    AniDB_Anime anidb_anime = ser.GetAnime(session);
                        ...
                        foreach (AnimeEpisode ep in epList)
                        {
                            if (dictAniEps.ContainsKey(ep.AniDB_EpisodeID))
                            {
                                    ...
                                    anidbep.EpisodeType == (int) enEpisodeType.Special)
                                {
                                    ...
                                    Contract_AnimeEpisode epContract = ep.ToContract(anidbep, new List<VideoLocal>(),
                                        userRecord, serUserRec);
            ...
            if (ep.EpisodeTypeEnum == enEpisodeType.Episode)
            {
                if (tvSummary != null && tvSummary.CrossRefTvDBV2 != null && tvSummary.CrossRefTvDBV2.Count > 0)
                {
                    ...
                    List<CrossRef_AniDB_TvDBV2> tvDBCrossRef =
                        Sorting.MultiSort<CrossRef_AniDB_TvDBV2>(tvSummary.CrossRefTvDBV2, sortCriteria);
